...
__global__ void fusedssimCUDA(
    ...
    float* __restrict__ dm_dmu2,
    float* __restrict__ dm_dsigma2_sq
) {
...
// ------------------------------------------
// Backward Kernel for img2: Apply chain rule to get
//    dL/d(img2) from partial derivatives
//    (dm_dmu2, dm_dsigma2_sq, dm_dsigma12)
//    and dL/dmap (the gradient from above).
// ------------------------------------------
__global__ void fusedssim_backward_img2_CUDA(
    int H,
    int W,
    int CH,
    float C1,
    ...
    const float* __restrict__ dm_dmu2,
    const float* __restrict__ dm_dsigma2_sq,
    const float* __restrict__ dm_dsigma12
) {
    auto block = cg::this_thread_block();
    ...
    // Shared memory for the fused data:
    // [0]: dm_dmu2*dL, [1]: dm_dsigma2_sq*dL, [2]: dm_dsigma12*dL
    __shared__ float sData[3][SHARED_Y][SHARED_X];
    __shared__ float sScratch[CONV_Y][CONV_X][3];
        ...
        // (1) Load + fuse multiplication
        {
            ...
            for (int row = warp_id; row < SHARED_Y; row += num_warps) {
                ...
                for (int col = lane_id; col < SHARED_X; col += 32) {
                    ...
                    float vs2   = get_pix_value(dm_dsigma2_sq,bIdx, c, gy, gx, CH, H, W);
                    float vs12  = get_pix_value(dm_dsigma12,  bIdx, c, gy, gx, CH, H, W);
...
//   Returns (ssim_map, dm_dmu1, dm_dsigma1_sq, dm_dsigma12, dm_dmu2, dm_dsigma2_sq).
...
// ------------------------------------------
// PyTorch Interface (Backward for img2)
//   Takes the gradient wrt the SSIM map and
//   the partial derivatives from forward;
//   returns dL/d(img2).
// ------------------------------------------
...
fusedssim_backward_img2(
    ...
    fusedssim_backward_img2_CUDA<<<grid, block>>>(
        H, W, CH, C1, C2,
        img1.contiguous().data_ptr<float>(),
        img2.contiguous().data_ptr<float>(),
        dL_dmap.contiguous().data_ptr<float>(),
        dL_dimg2.data_ptr<float>(),
        dm_dmu2.contiguous().data_ptr<float>(),
        dm_dsigma2_sq.contiguous().data_ptr<float>(),
        dm_dsigma12.contiguous().data_ptr<float>()
    );
