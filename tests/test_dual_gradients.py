...
def test_dual_gradient_flow():
    ...
    print(f"  SSIM value: {ssim_value.item():.6f}")
    print(f"  img1 grad mean: {img1.grad.mean().item():.6e}, std: {img1.grad.std().item():.6e}")
    print(f"  img2 grad mean: {img2.grad.mean().item():.6e}, std: {img2.grad.std().item():.6e}")
    print()
...
def test_gradient_symmetry():
    ...
    assert torch.allclose(ssim_ab, ssim_ba, rtol=1e-5, atol=1e-7), \
    ...
    assert torch.allclose(grad_a_from_ab, grad_a_from_ba, rtol=1e-4, atol=1e-6), \
        "Gradient of img_a should be consistent regardless of argument order"
    assert torch.allclose(grad_b_from_ab, grad_b_from_ba, rtol=1e-4, atol=1e-6), \
...
def test_numerical_gradient_img1():
    ...
    assert max_error < 0.1, f"Gradient error too large: {max_error}"  # 10% tolerance for finite differences
    print()
...
def test_different_batch_sizes():
    ...
    for batch_size in [1, 2, 4, 8]:
        ...
        assert img1.grad is not None, f"img1 should have gradients for batch_size={batch_size}"
        assert img2.grad is not None, f"img2 should have gradients for batch_size={batch_size}"
        assert not torch.isnan(img1.grad).any(), f"img1 gradients should be valid for batch_size={batch_size}"
        assert not torch.isnan(img2.grad).any(), f"img2 gradients should be valid for batch_size={batch_size}"
...
def test_different_image_sizes():
    ...
    for size in [16, 32, 64, 128]:
        ...
        assert not torch.isnan(img1.grad).any(), f"img1 gradients should be valid for size={size}"
        assert not torch.isnan(img2.grad).any(), f"img2 gradients should be valid for size={size}"
